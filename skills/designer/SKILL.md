...
## Обновление 11.09.2026 — форма по смыслу, не по шаблону

Старый режим «найти 2–3 места и нарисовать карточки» больше не считается достаточным.

Новый порядок:

1. Сначала прочитать метки редактора `[ВИЗУАЛ: …]`, если они есть.
2. Определить, что именно надо показать: процесс, сравнение, путь клиента, развилка, экран, письмо, диалог, антипример, maturity ladder, heatmap и т.д.
3. Проверить историю форматов сайта: не повторять одну и ту же форму без причины.
4. Выбрать форму из каталога, а не брать карточки по умолчанию.
5. В `notes` коротко объяснить выбор: почему эта форма подходит и почему не взята более очевидная.
6. Если схема не нужна — честно вернуть 0 схем с причиной. Схема через силу хуже, чем отсутствие схемы.
...
| Дизайнер | Какой формой это показать и как сверстать |
| Publisher | Проверяет, что дизайнерский шаг закрыт перед публикацией |

Каталог теперь шире базовых карточек. Используй не только `funnel`, `cards-num`, `cards-badge`, `stats`, но и:

- journey map / путь клиента дорожками;
- service blueprint / дорожки ролей;
- branching flow / цепочка с развилками;
- maturity ladder / лестница зрелости;
- scorecard / оценочная карта;
- decision matrix / матрица выбора;
- quadrant / квадрант с точками;
- radar / радар;
- heatmap / тепловая сетка;
- feedback loop / петля обратной связи;
- data-flow map / карта потоков данных;
- object anatomy / анатомия объекта;
- annotated screen / размеченный экран;
- teardown / разбор примера;
- anti-pattern map / карта антипаттернов;
- benchmark bars / полосы против нормы;
- share grid / сетка долей;
- waterfall / водопад вклада;
- website mockup, CRM screen, bot dialogue, email teardown, dashboard snapshot, before/after screen.

Правило: у второго поколения единица схемы не карточка со списком, а отношение: дорожка, ступень, клетка, звено петли, выноска. У третьего — предмет: страница, экран, переписка, письмо, отчёт.

### Живые примеры и скриншоты

Чужой скриншот не брать. Нет прав на кадр чужого продукта, письма или переписки.

Если нужен пример с рынка:

- рисовать синтетический мокап в стиле проекта;
- подписывать как «синтетический пример по рыночному паттерну»;
- указывать `refs`: на какие открытые источники/паттерны смотрели;
- реальные клиентские цифры в мок не ставить.

Свой кадр можно использовать только если это наш продукт/клиент с разрешением. Тогда указать `origin: "own"` и `origin_note`.
name: designer
